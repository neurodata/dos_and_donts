...
def main(task_index):
    ...
    args = [dict(m=m, var_delta=var_delta) for m, var_delta in product(ms, var_deltas)][task_index::2]
    ...
    res_df.to_csv(
        f"./results/20200321_weighted_correct_nodes_{task_index}.csv", index=False
    )
